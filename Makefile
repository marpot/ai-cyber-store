.PHONY: up down build restart logs ps frontend backend clean typecheck lint lint-fix test test-pytest test-coverage help
...
help:
	...
	@echo "  make test-pytest        Run pytest in the recommendation-service"
...
test-pytest:
	docker compose exec recommendation-service python -m pytest

test-coverage:
	docker compose exec recommendation-service python -m pytest --cov=app --cov-report=term-missing
